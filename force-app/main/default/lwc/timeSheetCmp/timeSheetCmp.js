...
export default class TimeSheetCmp extends LightningElement {
  ...
  wiredTimesheetDays(results) {
    ...
    if (data) {
      ...
      this.timesheetDaysPerWeek = this.timesheetDays.filter(
        (day) => day.weekNumber === this.activeWeekNumber
      );
    } else if (error) {
      this.dispatchEvent(
        new ShowToastEvent({
  ...
  changeWeek(event) {
    ...
    if (this.timesheetDays) {
      ...
      let daysPerWeek = this.timesheetDays.filter(
        (day) =>
          day.weekNumber === event.detail.weekNumber + 1 && !day.deleteFlag
      );

      if (daysPerWeek.length > 0) {
        this.timesheetDaysPerWeek = daysPerWeek;
      } else {
        let deletedDaysPerWeek = this.timesheetDays.filter(
          (day) => day.weekNumber === event.detail.weekNumber + 1
        );
  ...
  handleChangeValue(event) {
    ...
    let index = this.timesheetDays.findIndex(
      (earning) => earning.id === event.detail.earningsId
    );
    if (dayId !== "") {
      let hoursindex = this.timesheetDays[index].hours.findIndex(
        (day) => day.id === dayId
      );
      newTimesheetDays[index].hours[hoursindex].hours = event.detail.value;
    } else {
      let hoursindex = this.timesheetDays[index].hours.findIndex(
        (day) => day.day === event.detail.day
      );
      if (hoursindex > -1) {
        newTimesheetDays[index].hours[hoursindex].hours = event.detail.value;
      } else {
  ...
  handleRemoveEarning(event) {
    ...
    if (event) {
      ...
      let perWeekIndex = this.timesheetDaysPerWeek.findIndex(
