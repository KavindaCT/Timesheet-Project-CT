...
export default class Earning extends LightningElement {
  ...
  renderedCallback() {
    var total = 0;
    if (this.earning) {
      this.template.querySelector(`[data-id="earning-type"]`).value =
        this.earning.earningType;
      /*
            for(let i = 0; i < 7; i++) {
      ...
      for (let i = 1; i < 6; i++) {
        if (this.earning.hours.length > 0) {
          // i = 5
          console.log("dcsccasc" + this.earning.hours.length);
          let earningHours = this.earning.hours.find(
            (earning) => earning.day.charAt(0) === i.toString()
          );
          if (earningHours) {
            this.template.querySelector(`[data-id="${i}"]`).value =
              earningHours.hours;
  ...
  handleEarningChange(event) {
    ...
    if (value !== null && value !== undefined) {
      this.dispatchEvent(
        new CustomEvent("changeearning", {
          detail: { earningType: value, id: this.earning.id }
        })
      );
    }
  }
  ...
  handleRemoveEarning() {
    const removeEarning = new CustomEvent("remove", {
      detail: { id: this.earning.id }
    });
