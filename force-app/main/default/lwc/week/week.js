...
export default class Week extends LightningElement {
  ...
  set currentWeek(value) {
    ...
    for (let i = 1; i < 7; i++) {
      ...
      } else if (weekStart.getDay() <= i && i <= weekEnding.getDay()) {
        // 3 <= 6 && 6 <= 5
        date.setDate(weekStart.getDate() + (i - weekStart.getDay()));
  ...
  changeValue(event) {
    ...
    if (dayId !== "") {
      this.dispatchEvent(
        new CustomEvent("changevalue", { detail: event.detail })
      );
    } else {
      ...
      this.dispatchEvent(
        new CustomEvent("changevalue", {
          detail: {
            earningsId: event.detail.earningsId,
            dayId: "",
            day: weekDay,
            date: date,
            name: name,
  ...
  handleChangeEarningType(event) {
    ...
    this.dispatchEvent(
      new CustomEvent("changeearningtype", { detail: detail })
    );
  }

  removeEarning(event) {
    this.dispatchEvent(
      new CustomEvent("removeearning", { detail: event.detail })
    );
  }
}
