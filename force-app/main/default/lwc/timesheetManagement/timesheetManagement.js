...
export default class TimesheetManagement extends LightningElement {
  ...
  wiredPreviousTimesheets({ error, data }) {
    if (data) {
      this.previousTimesheets = data.filter(
        (timesheet) => timesheet.Status__c !== "Draft"
      );
  ...
  get timesheetsToShow() {
    return (
      this.previousTimesheets !== undefined &&
      this.previousTimesheets !== null &&
      this.previousTimesheets.length > 0
    );
  }
  ...
  viewTimeSheet(event) {
    if (event) {
      this.hideModalBox();
      this.dispatchEvent(
        new CustomEvent("setview", { detail: { view: "timesheet" } })
      );
    }
  }
